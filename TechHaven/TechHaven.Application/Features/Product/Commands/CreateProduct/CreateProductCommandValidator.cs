...
public class CreateProductCommandValidator : AbstractValidator<CreateProductCommand>
{
  ...
  {
      // QUY TẮC CHUNG (Data Integrity) - Áp dụng cho cả Draft == false || true
    ...
    RuleFor(x => x.BatteryCapacity)
      ...
      .When(x => x.BatteryCapacity.HasValue);
        ...
        //  QUY TẮC CHẶT CHẼ - Chỉ áp dụng khi PUBLISH (IsDraft = false)
        When(x => x.IsDraft == false, () =>
        {
            RuleFor(x => x.BrandName)
                .NotEmpty().WithMessage("Brand name is required for published products.");

            // Logic giá bán phải lời hơn giá vốn (Chỉ check khi chính thức bán)
            // Khi là Draft, lỡ user nhập giá vốn trước, giá bán sau thì không nên chặn
            RuleFor(x => x.SellPrice)
